<div class="w-full">
    ...
    <div class="">
        ...
        <div class="col-span-12 w-full">
            ...
            <div id="menu-date"
                class="w-full flex overflow-x-scroll pb-8 pl-2 -ml-2 lg:pl-0 lg:-ml-0 lg:pb-0 lg:overflow-x-visible">
                ...
                @foreach ($period as $date)
                    <div id="{{ $date }}" class="{{ $date == $menu_date ? 'flex' : 'lg:hidden flex' }} mr-4">
                        <a @if ($date !== $menu_date) href="{{ route('welcome') }}?menu={{ base64_encode(serialize(['menu_date' => $date])) }}" @endif>
                            <div
                                class="bg-base-200 shadow-lg w-32 block rounded-lg col-span-2 overflow-hidden text-center">
                                <div
                                    class="py-3 {{ $menu_date === $date ? 'bg-primary' : ' bg-primary-focus cursor-pointer hover:shadow-lg' }}">
                                    <h3 class="text-white text-md">{{ date('M', strtotime($date)) }}</h3>
                                </div>
                                <div class="py-3">
                                    <h2 class="text-2xl text-base-content font-bold">{{ date('d', strtotime($date)) }}
                                    </h2>
                                </div>
                                        ...
                                        <div class="avatar">
                                            <div class="w-16 h-16 p-1 mask mask-squircle bg-base-100">
                                                <img class="mask mask-squircle"
                                                    src="{{ $bento->product->image_file ? $bento->product->image_file : 'https://www.kenyons.com/wp-content/uploads/2017/04/default-image-620x600.jpg' }}">
                                            </div>
                                        </div>
                                    </div>
        ...
        @if (count($products) > 0)
            ...
            @foreach ($products as $product)
                <div wire:loading.remove wire:loading.target="changeBrand"
                    class="col-span-6 md:col-span-4 lg:col-span-4 xl:col-span-3 md:flex pb-8 w-full indicator">
                    <div class="card bordered shadow-lg w-full rounded-box bg-base-200">
                        <figure class="px-4 pt-4">
                            <img src="{{ $product->image_file ? $product->image_file : 'https://www.kenyons.com/wp-content/uploads/2017/04/default-image-620x600.jpg' }}"
                                class="h-40 object-cover object-center rounded-box bg-cover bg-center">
                        </figure>
                        <div class="card-body h-30 px-5 pt-4 pb-0">
                            ...
                            <p class="hidden lg:block text-xs mt-2">
                                {{ mb_strimwidth($product->description, 0, 50, '...') }}</p>
                        </div>
                        <div class="pb-4 px-5 w-full mt-3 justify-between">
                            ...
                            @auth
                                <button wire:click="addToCart({{ $product->id }},'{{ $menu_date }}')"
                                    class="btn btn-primary btn-block w-full btn-sm text-sm m-0 rounded-lg">{{ __('Add To Cart') }}</button>
                            @else
                                <a href="{{ route('login') }}"
                                    class="btn btn-primary btn-block btn-sm text-sm m-0 rounded-lg">{{ __('Add To Cart') }}</a>
                            @endauth
                        </div>
                    </div>
                    <!-- ordered -->
                    @auth
                        ...
                        @endphp
                        @if ($ordered)
                            <div class="indicator-item badge badge-info uppercase text-xs py-4"
